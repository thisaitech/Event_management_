...
/**
 * Create booking request (no auth required - guests can book)
 */
export const createBooking = async (bookingData: CreateBookingData): Promise<BookingRequest> => {
  const response = await fetch(`${API_URL}/api/bookings`, {
    method: 'POST',
    headers: getHeaders(false), // Changed to false - no auth required for booking creation
...
export const addSubscriber = async (email: string, phone?: string): Promise<Subscriber> => {
  try {
    ...
    return await handleResponse<Subscriber>(response);
  } catch (error) {
    // Handle network errors or fetch failures
    if (error instanceof TypeError && error.message.includes('fetch')) {
      throw new Error('Cannot connect to server. Please make sure the backend server is running on http://localhost:3001');
...
export const deleteSubscriber = async (identifier: string): Promise<void> => {
  const response = await fetch(`${API_URL}/api/subscribers/${identifier}`, {
    method: 'DELETE',
    headers: getHeaders(true),
  });
  if (!response.ok) {
